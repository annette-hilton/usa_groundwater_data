...
2. Raw text files from each data source are saved directly from the web browser (avoiding copy/paste potential errors). The two files are saved as:
  - "State_Raw_Date", where the State is the state name and the date is the date the data was obtained (ex. "California_Raw_11_08_19") 
...
1. The two raw text files for each state are copied
2. The copied files are renamed: 
  - "state_raw_import"
  - "state_raw_other_import" 
3. The files are opened and the metadata information prior to the collection data is manually removed for ease of import into RStudio 
