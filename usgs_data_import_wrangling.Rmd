...
4. **In Progress** Write each individual final state dataframe to a text file or csv file
...
# Create the list of names that the loop will use later to populate the names of each dataframe 
...
# Attempt to parse dates/read column as date (unsure if this was successful) 
...
# Write the resulting dataframes of all 53 state/territories to csv files and tsv files, saved in separate folders 
  ...
  write_csv(results, here::here("merged_state_data_csv", file = paste0(df.names[i], ".csv")))
  write_tsv(results, here::here("merged_state_data_tsv", file = paste0(df.names[i], ".txt")))
  }

# IN PROGRESS: 
# Currently attempting to figure out how to write individual text or csv files in the loop. Code attempts below. 

  # final_state_files <- file.path(here::here("merged_data_files"), paste0(results, "_", i, ".txt")) 
  # 
  # write.table(results, file = final_state_files, sep = "", row.names = FALSE, col.names = TRUE, quote = FALSE)    
# save(results, list = df.names[i], file = "state.RData")
...
### Functions to tidy and wrangle data 
...
```{r}
# entire_usa <- join_all(results, by="site_no", type = "full") 
```
