...
arc_join_me_tidy <- arc_join_me %>% 
  ...
  select(site_no, dec_lat_va.x, dec_long_va, well_depth_va, hole_depth_va, alt_va, vkbg_surf, ucaq_surf, mccu_surf, mcaq_surf, lccu_surf, lcaq_surf, mwaq_surf, lwaq_surf, mdwy_surf)
...
```{r}
# Add a column that is the average of well depth and hole depth 
# Add a column that is elevation minus well depth

me_analysis <- arc_join_me_tidy %>% 
  mutate(avg_depth = (well_depth_va + hole_depth_va) / 2) %>% 
  mutate(final_depth = alt_va - avg_depth)
   
  
  # mutate(true_depth = alt_va - 
```
